...
开启自动分类后，正常完成首轮 chat run 的 session 默认必须归入某个分类，不应把 `noop` 作为常态结果留在 `未分组`。
...
其中 `noop` 只作为防御性返回保留，不应成为正常首轮会话的最终落点。若 Agent 返回 `noop`、返回无效分类，或 provider 调用失败，系统应把该会话归入一个 holding category，例如：
