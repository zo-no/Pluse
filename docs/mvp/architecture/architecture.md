...
### 删除即归档
Quest、Todo、Project 的用户删除操作统一视为软删除/归档。默认 active 视图只显示 `deleted=false` 的对象；归档内容只在专门的归档区展示，并支持恢复。
...
| UI：Session 列表 + Task 列表（两套独立入口） | UI：左侧会话列表（kind='session'）+ 右侧统一任务面板（Quest task + Todo） |
