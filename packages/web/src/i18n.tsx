...
const EN_MESSAGES: Record<string, string> = {
  ...
  提醒: 'Reminder',
  打卡: 'Check-in',
  打卡详情: 'Check-in details',
  打卡备注: 'Check-in note',
  打卡标题不能为空: 'Check-in title is required',
  完成打卡: 'Complete check-in',
  延后打卡: 'Snooze check-in',
  新建打卡: 'New check-in',
  创建打卡: 'Create check-in',
  输入打卡标题: 'Enter check-in title',
  补充打卡内容: 'Add check-in details',
  暂无打卡内容: 'No check-in content yet',
  '可选，记录这次实际情况': 'Optional, record what actually happened',
  现在: 'Now',
  逾期: 'Overdue',
  提醒详情: 'Reminder details',
  ...
  无指定时间: 'No scheduled time',
  暂无提醒内容: 'No reminder details',
  ...
  '当前范围暂无打卡。': 'No check-ins in this scope yet.',
  ...
  全局: 'Global',
  通知: 'Notifications',
  '会话完成后创建待办': 'Create todo after session completes',
  'AI 完成一轮会话后，自动在待办列表创建提醒': 'After an AI session run completes, create a review reminder in the todo list.',
  '会话失败后创建待办': 'Create todo after session fails',
  'AI 执行出错时，自动在待办列表创建提醒': 'When an AI run fails, create a reminder in the todo list.',
  '会话完成后语音播报': 'Voice announcement after session completes',
  '检测中…': 'Checking…',
  '已安装 {{version}}，可用 kairos config set voice Tingting 切换音色': 'Installed {{version}}. Use kairos config set voice Tingting to change the voice.',
  '由 GitHub 版 kairos 驱动，可用 kairos config set voice Tingting 切换音色': 'Powered by the GitHub version of kairos. Use kairos config set voice Tingting to change the voice.',
  '从 GitHub 下载并安装 kairos 后可启用语音播报': 'Download and install kairos from GitHub to enable voice announcements.',
  '从 GitHub 安装': 'Install from GitHub',
  '安装中…': 'Installing…',
  '元数据补全': 'Metadata completion',
  '用于在首轮会话结束后补全会话的辅助元数据。': 'Completes supporting session metadata after the first run.',
  '首轮会话后自动分类': 'Auto-classify after first session run',
  '首个 chat run 完成后，Agent 会尝试复用或新建会话分类。': 'After the first chat run, the agent will try to reuse or create a session category.',
  '作用于所有项目。项目级 Prompt 在各项目设置里单独编辑。': 'Applied to all projects. Project-level prompt can be configured on each project page.',
  'CLI 集合': 'CLI catalog',
  '设置一条用于查询当前环境所有外部 CLI 指令的命令行。Pluse 只会把它注入 Quest 上下文，不执行或校验。':
    'Set one command line that lists all external CLI commands available in this environment. Pluse only injects it into Quest context; it does not execute or verify it.',
  查询命令: 'Query command',
  编辑: 'Edit',
