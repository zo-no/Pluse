...
describe('settings API', () => {
  ...
  it('injects the CLI catalog command into session and task prompts', async () => {
    ...
    for (const prompt of [sessionPrompt, taskPrompt]) {
      ...
      expect(prompt).toContain('信息降噪规则')
      expect(prompt).toContain('满足任一条件就先创建完整 progress，再开始执行')
      expect(prompt).toContain('已有未完成项就续写，不要重复建计划。')
      expect(prompt).toContain('先创建 2-4 个中层阶段')
      expect(prompt).toContain('避免一开始拆成大量待开始条目')
      expect(prompt).toContain('Progress 条目 = 用户关心的阶段目标')
      expect(prompt).toContain('微动作（读文件、跑命令、安装依赖等）只写进 `active-form`')
      expect(prompt).toContain('自然完成时默认只更新现有 progress 并回复用户')
      expect(prompt).toContain('避免制造信息爆炸')
      expect(prompt).toContain('默认停止创建新的 Progress/Todo/Reminder/Check-in')
      expect(prompt).toContain('每完成一个步骤，都要重新判断“整个任务是否已经完成”')
      expect(prompt).toContain('只有当用户提出新的目标，或当前目标明显扩展到超出原计划时，才开启下一轮 progress 流程')
      expect(prompt).toContain('用户在聊天中调整方案、顺序或范围时，优先修正当前未完成的 future steps')
      expect(prompt).toContain('如果任务进行中用户发来新消息改变方向，优先调整当前计划和执行路径')
      expect(prompt).toContain('每次只允许一个步骤处于 `doing`。')
      expect(prompt).toContain('每个实现类步骤后都必须有验证步骤。')
      expect(prompt).toContain('分析 / 实现 / 验证')
      expect(prompt).toContain('create --for human` 作为默认 Progress')
    ...
    expect(sessionPrompt).toContain('自然完成时默认用回复收尾')
    expect(taskPrompt).toContain('自然完成时默认用运行结果收尾')
